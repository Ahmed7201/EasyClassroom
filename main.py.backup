    ...
    if st.session_state.selected_course_id is None:
        ...
        for idx, course in enumerate(course_data):
            with cols[idx % 3]:
                ...
                label = f"📘 {course['name']}\n\n{deadline_str}\n🚨 {course['pending_count']} Pending | 📂 {len(course['works'])} Items"
        ...
        with tab_tasks:
            ...
            with col2:
                ...
                if final_weight > 0:
                    ...
                    if needed > 100:
                        st.error(f"You need {needed:.1f}% (Impossible 😢)")
                    elif needed < 0:
                        st.success(f"You need {needed:.1f}% (Already passed! 🎉)")
                    else:
                        st.warning(f"You need **{needed:.1f}%** on the final.")

        # --- TAB: RESOURCES ---
        with tab_resources:
            st.subheader("📚 Course Materials Library")
